...
import { ArrowLeft, Clock, Star, CheckCircle, Clock as ClockIcon, Calendar, User, ChevronLeft } from 'lucide-react';
...
const SkillCard = ({ title, partnerName, date, duration, rating, status, type, onStatusChange }: SkillCardProps) => {
  const statusColors = {
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <button 
          onClick={(e) => {
            e.stopPropagation();
            const statuses: SkillStatus[] = ['scheduled', 'ongoing', 'completed'];
...
const SkillHistoryPage = () => {
  const [activeTab, setActiveTab] = useState<'all' | 'learned' | 'taught'>('all');
    ...
    <div className="min-h-screen bg-[#F0F6FF]">
      ...
      <header className="bg-white shadow-sm p-4 sticky top-0 z-10">
        <div className="flex items-center">
          ...
          <h1 className="text-lg font-semibold text-gray-900 ml-2">Skill History</h1>
        </div>
      </header>
      <div className="p-4">
        {/* Search bar */}
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Completed Trades</h3>
              <p className="text-sm text-gray-500">Total skills you've completed</p>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 text-center">
            ...
            <div className="text-2xl font-semibold text-cyan-900">{skills.length}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 text-center">
            <div className="text-sm font-medium text-gray-500 mb-1">Learned</div>
        ...
        <div className="inline-flex rounded-lg bg-gray-100 p-1 w-full">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as 'all' | 'learned' | 'taught')}
