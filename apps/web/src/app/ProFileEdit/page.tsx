...
import { Settings, Edit2, ChevronRight, MapPin, Star, Zap, User, Plus, X, Save, Filter, MessageSquare, Bell, Compass } from "lucide-react";
...
const NavItem = ({ icon: Icon, label, isActive }: NavItemProps) => (
    <div className={`flex flex-col items-center p-1 cursor-pointer transition-colors ${isActive ? 'text-primary' : 'text-gray-500'}`}>
...
interface InteractiveSkillBadgeProps {
  skill: string;
  type: 'skill' | 'learning';
  handleRemoveSkill: (skillToRemove: string, type: 'skill' | 'learning') => void;
}

const InteractiveSkillBadge = ({ skill, type, handleRemoveSkill }: InteractiveSkillBadgeProps) => {
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Această acțiune va șterge permanent abilitatea/interesul <strong>{skill}</strong> din profilul tău.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const ProfileActionButton = ({ icon: Icon, text, onClick }: ProfileActionButtonProps) => (
  <Button
    variant="ghost"
...
export default function ProfileScreenInteractive() {
  ...
  const handleRemoveSkill = (skillToRemove: string, type: 'skill' | 'learning') => {
    if (type === 'skill') {
    ...
    <div className="flex min-h-screen justify-center bg-gray-50 pb-20">
      ...
      <div className="w-full max-w-sm bg-background shadow-xl overflow-y-auto relative">
        ...
        <div 
          ...
          style={{ backgroundImage: `url('https://placehold.co/390x256/1d324e/ffffff?text=Banner+Image')` }}
        >
        ...
        <div className="relative p-6 -mt-20"> 
          ...
          <div className="flex flex-col items-center">
            <Avatar className="h-40 w-40 border-4 border-background shadow-lg">
              <AvatarFallback className="bg-secondary text-white font-bold text-4xl">UT</AvatarFallback>
            </Avatar>
            <h1 className="text-2xl font-bold text-primary mt-4">{initialUserData.name}</h1>
            <p className="text-sm text-muted-foreground mt-1">{initialUserData.role}</p>
            <div className="flex items-center space-x-4 mt-3">
                ...
                <Card className="flex items-center space-x-1 p-2 bg-card rounded-full shadow-md border-none">
                    ...
                    <span className="text-base font-medium text-primary">{initialUserData.rating}</span>
                </Card>
            </div>
          </div>
          ...
          <Card className="mt-8 p-4 bg-muted/40 rounded-xl border-none shadow-none">
            <div className="flex justify-between items-start mb-3">
              ...
              <Button variant="ghost" size="sm" onClick={() => setIsEditingDescription(prev => !prev)}>
                {isEditingDescription ? <X className="h-4 w-4" /> : <Edit2 className="h-4 w-4" />}
              </Button>
            </div>
            ...
            {isEditingDescription ? (
              <div className="space-y-2">
                <Textarea 
                  ...
                  rows={4}
                />
                <Button onClick={handleDescriptionSave} size="sm" className="w-full">
          ...
          <Card className="mt-6 p-4 bg-muted/40 rounded-xl border-none shadow-none">
            ...
            <div className="flex flex-wrap gap-2">
              {skills.map(skill => (
                <InteractiveSkillBadge key={skill} skill={skill} type="skill" handleRemoveSkill={handleRemoveSkill} />
              ))}
            </div>
            {isAddingSkill && (
                <div className="flex space-x-2 mt-4">
                    <Input 
                        ...
                        className="bg-white"
                    />
                    <Button onClick={() => handleAddSkill('skill')} className="bg-green-500 hover:bg-green-600 px-4">
                        Adaugă
                    </Button>
                    <Button onClick={() => setIsAddingSkill(false)} variant="ghost" className="px-3">
                        <X className="h-5 w-5" />
                    </Button>
                </div>
            )}
          ...
          <Card className="mt-6 p-4 bg-muted/40 rounded-xl border-none shadow-none">
            <div className="flex justify-between items-start mb-3">
              <h3 className="text-base font-semibold text-primary">Looking to learn:</h3>
              <Button 
                size="icon" 
                className="h-8 w-8 rounded-lg bg-[#24507f] hover:bg-[#24507f]/90"
                onClick={() => {
            ...
            <div className="flex flex-wrap gap-2">
              {learning.map(skill => (
                <InteractiveSkillBadge key={skill} skill={skill} type="learning" handleRemoveSkill={handleRemoveSkill} />
              ))}
            </div>
            {isAddingLearning && (
                <div className="flex space-x-2 mt-4">
                    <Input 
                        ...
                        className="bg-white"
                    />
                    <Button onClick={() => handleAddSkill('learning')} className="bg-green-500 hover:bg-green-600 px-4">
                        Adaugă
                    </Button>
                    <Button onClick={() => setIsAddingLearning(false)} variant="ghost" className="px-3">
                        <X className="h-5 w-5" />
                    </Button>
                </div>
            )}
